...
// thinkBudgetForShow reports a model's think_budget parameter and what it
// resolves to, against whichever options the caller asked about: the model's
// own, or those overridden on the request.
//
...
// for. A model whose options set neither num_predict nor num_ctx has no window
// to resolve against here — the runtime default supplies one only once a
