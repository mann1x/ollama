...
// closing tag rather than failing the request, and its qwen3-coder parser
// hands an unparseable block back as content instead of erroring at all
// (mann1x/cline#54). The subject of these tests is what routes.go does when a
// parser errors mid-stream, not which inputs a parser rejects, so they use a
// parser that still has that error to raise.
